...
public class SVM extends Frame {
	...
	public SVM(){
		...
		setResizable(false);	
		setBackground(settings.background_color_default);
	...
	public boolean handleEvent(Event e){
		if(e.id==Event.WINDOW_DESTROY){
			...
			String label = (String)e.arg;

			if("Exit".equals(label)){
				System.exit(0);
			} else if("Load Input Data".equals(label)){
				ind.loadInputData();			
				return true; 
			} else if("Median".equals(label)){
				if(ind.V != null){
					if(algorithm !=null){algorithm.stop_(); algorithm = null; init2();}	
					algorithm = new Median(this);
					control.show();
					mb.getMenu(2).getItem(0).setLabel("Hide Simulation Control");
				}
				return true;
			} else if("Perceptron".equals(label)){
				if(ind.V != null){
					if(algorithm !=null){algorithm.stop_(); algorithm = null; init2();}	
				...
				try{
					...
					System.out.println("Eroare la deschiderea PDF!");
				}
				return true; 
