		...
		if(svm.ind.input_file == null){
			...
			if(svm.algorithm == null){
				...
				if(start.getLabel().startsWith("Start Simulation")){
					if(init){
						// Prima rulare: pornim algoritmul curent
						svm.algorithm.start_simulation();
						init = false;
					}else{
						// Reluare dupa Stop: cream instanta noua si pornim
						svm.algorithm = svm.algorithm.newInstance();
						svm.algorithm.start_simulation();
					}
					start.setLabel("Stop Simulation");
				}else{
					// Stop: opreste thread-ul, reseteaza canvas si linia
					svm.algorithm.stop_();
					svm.design.show_line = false;
					svm.design.calculates = false;
					svm.design.repaint();
					start.setLabel("Start Simulation");
					init = false; // permite reluarea cu newInstance
				}			
			}
		}		
	}
